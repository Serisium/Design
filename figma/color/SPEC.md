...
| Layer | Collection | Shape | Notes |
|---|---|---|---|
| Ramp primitives | `Sanzō Wada Tones` — new, published | `Red/Burnt Sienna/600` | Sibling of Base in the same library file |
| Slot grid | `Sanzō Wada Palette`, local | `Slot/2` (seed, unchanged) · `Slot/2/600` | Aliases into Tones; rewritten on apply |
| Roles | `Sanzō Wada Palette`, local | `Role/Header` — semantic, tone-free | Tone lives in the alias, never the name |

- **Tones is a separate collection, not additions to Base.** Base's identity is "fixed
  historical values, never republished"; 159 × 11 generated variables whose values change
  whenever the ramp algorithm is retuned would destroy that. Same library file, so no extra
  enable step for consumers. Each seed's natural rung **aliases into Base** rather than
  duplicating the hex — "this rung is the printed colour" stays inspectable. Variable
  descriptions carry the L\* and the seed marker.
- **`Slot/N` still means the seed**, so every existing role alias keeps its meaning.
...
- Apply writes 4 seed + 44 rung aliases — still constant in role count. Lazy import grows
  from at most 4 of 159 to at most 48 of ~1,750.
